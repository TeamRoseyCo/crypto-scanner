...
CONFIG = {
    ...
    'WEIGHTS': {
        'atr_trend': 2.0,
        'volume_surge': 1.5,
        'rs_vs_btc': 2.5,        # Most predictive
        'accumulation': 2.0,
        'breakout_quality': 1.5,
        'rsi_reversal': 1.0,
        'macd_cross': 1.0,
        'dmi_bullish': 1.0,
        'candle_strength': 1.0,
        'price_above_sma': 1.0,
        'intraday_momentum': 1.5,
    },
}
...
def compute_accumulation_distribution(high, low, close, volume):
    ...
    """
    if 'volume' not in volume.name and not isinstance(volume, pd.Series):
...
def apply_prime_key_filter(coins_data, btc_7d):
    ...
    for i, coin in enumerate(coins_data):
        ...
        try:
            ...
            # Alpha requirements (most should pass)
            alpha_checks = [passes_rs, passes_accumulation, passes_momentum, passes_atr_trend]
...
if __name__ == "__main__":
    ...
    if not df.empty:
        ...
        detailed_file = "rspS_prime_key_detailed.csv"
