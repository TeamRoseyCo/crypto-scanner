...
# Tiered SIDEWAYS exceptions — SIDEWAYS ≠ full block.
# When ≥3 scanners agree (CONVERGENCE) or 2 scanners agree with a perp signal
# (institutional accumulation evidence), allow a small entry despite SIDEWAYS.
# Rationale: coins with individual catalysts decouple from BTC regime.
SIDEWAYS_EXCEPTION_CONFIG = {
    "convergence_max_pos_pct":  3.0,   # 3% cap (vs 6% normal SIDEWAYS, 12% BULL)
    "strong_perp_max_pos_pct":  2.0,   # 2% cap, requires perp scanner firing
    "account_size_default":  97_000,   # fallback for plan calculation
}
...
def _build_exception_plan(v: CoinView, max_pos_pct: float) -> Optional[dict]:
    """Recalculate position sizing for a SIDEWAYS exception entry at reduced cap."""
    if not v.trade_plan or not v.price:
        return None
    tp    = v.trade_plan
    entry = tp.get("entry") or v.price
    stop  = tp.get("stop")
    if not stop or not entry or stop >= entry:
        return None
    acct          = SIDEWAYS_EXCEPTION_CONFIG["account_size_default"]
    stop_dist_pct = abs((stop - entry) / entry) * 100
    max_pos_usd   = acct * max_pos_pct / 100
    risk_usd      = max_pos_usd * stop_dist_pct / 100
    return {
        "entry":         entry,
        "stop":          stop,
        ...
        "risk_usd":      risk_usd,
        "take_profits":  tp.get("take_profits") or [],
    }
...
def build_text_report(
    ...
    # ── SIDEWAYS EXCEPTIONS ──────────────────────────────────────────────────
    # CONVERGENCE (3+ scanners) → 3% cap.  STRONG_SETUP with perp → 2% cap.
    # Coins with individual catalysts decouple from BTC regime; these warrant
    # a small entry despite SIDEWAYS, with tighter sizing as the guard rail.
    if context.get("regime", "").lower() == "sideways":
        exc_coins: list[tuple] = []
        for v in convergence:
            exc_coins.append((v, "CONVERGENCE", SIDEWAYS_EXCEPTION_CONFIG["convergence_max_pos_pct"]))
        for v in strong_setups:
            if v.perp_tier is not None:
                exc_coins.append((v, "STRONG+PERP", SIDEWAYS_EXCEPTION_CONFIG["strong_perp_max_pos_pct"]))
        if exc_coins:
            lines += [
                "",
                "  ┌──────────────────────────────────────────────────────────────────┐",
                "  │  ⚡ SIDEWAYS EXCEPTIONS  —  individual catalyst, small entry OK    │",
                "  │  CONVERGENCE (≥3 scanners) → 3% pos cap                         │",
                "  │  STRONG+PERP (OI/funding signal) → 2% pos cap                   │",
                "  │  Use tighter stops. Half your normal size. BTC drag still real.  │",
                "  └──────────────────────────────────────────────────────────────────┘",
            ]
            for idx, (v, exc_type, max_pct) in enumerate(exc_coins, 1):
                plan = _build_exception_plan(v, max_pct)
                badges = "  ".join([
                ...
                lines.append(
                    ...
                    f"{badges}    {price_str}  24h={chg_str}  → {exc_type}"
                )
                lines.append(f"       {_signals_summary(v)}")
                if plan:
                    tp_strs = [f"${t.get('price', 0):.6f}" for t in plan["take_profits"][:3]]
                    lines.append(
                        f"       Exception plan: entry ${plan['entry']:.6f}  |  "
                        f"stop ${plan['stop']:.6f} -{plan['stop_dist_pct']:.2f}%  |  "
                        f"max pos ${plan['max_pos_usd']:,.0f} ({plan['max_pos_pct']:.0f}%)  |  "
                        f"risk ~${plan['risk_usd']:,.0f}"
                    )
                    if tp_strs:
                        lines.append(f"       TPs: {', '.join(tp_strs)}")
                else:
                    lines.append("       No trend plan — check chart for entry/stop before sizing")
            lines.append("")
...
def run(
    ...
    json_ts     = _OUTPUT_DIR / f"master_radar_{ts_file}.json"
